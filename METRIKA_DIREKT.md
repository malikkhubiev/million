...
1. Метрика → счётчик `112323537` → **Настройки**.
2. Включи **Вебвизор**, **Карту кликов**, **Электронную коммерцию**.
3. Токен Measurement Protocol — **не** страница «Код счётчика» (там только JS для сайта, код уже стоит на Vercel).
   Путь:
   - левое меню: **Настройки** (шестерёнка у счётчика)
   - пролистай страницу **вниз** мимо «Код счётчика»
...
Номера `headline_*` не подряд — так на сайте (`data-headline`).
...
### 2.1. Куда что писать в Директе

**Ссылка объявления:** только `https://life-energy-phi.vercel.app/` (без UTM).

**Параметры URL** — только у **каждого объявления**, целиком строка из таблицы ниже.  
Не пиши UTM в кампании и в группе: иначе метки дублируются или затирают друг друга.

`{keyword}` — подстановка Директа, не заменяй вручную.

Шаблон с `CAMPAIGN` / `AD` ниже — только пример смысла, **не вставляй его как есть**:

```
utm_source=yandex&utm_medium=cpc&utm_campaign=CAMPAIGN&utm_content=AD&utm_term={keyword}
```
...
1. У каждого из 9 объявлений — своя полная строка UTM в **Параметрах URL** (разный `utm_content`). Кампания и группа — без UTM.
