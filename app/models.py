...
class Client(Base):
    ...
    # Диагностика: 0 idle, 1–3 ждём ответ, 4 готово к «Расскажи»
    diag_step: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    diag_answer_1: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    diag_answer_2: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    diag_answer_3: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    diag_text: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    diag_status: Mapped[str] = mapped_column(String(16), nullable=False, default="none")
    diag_shown: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    diag_awaiting_reveal: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
...
class Payment(Base):
    ...
    product_code: Mapped[str] = mapped_column(String(32), nullable=False, default="program", index=True)
...
class BehaviorVisit(Base):
    ...
    clicked_telegram_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    bot_started: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    bot_started_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    show_phone: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    show_phone_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    payment_started: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    payment_started_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    payment_success: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    payment_success_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
